...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    """
    clean_dish = set(dish_ingredients)
    return (dish_name,clean_dish)
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    """
    columns = [('VEGAN',VEGAN),('VEGETARIAN',VEGETARIAN),('PALEO',PALEO),('KETO',KETO),('OMNIVORE',OMNIVORE)]
    for name,col in columns:
        if dish_ingredients.issubset(col):
...
def tag_special_ingredients(dish):
    ...
    name = dish[0]
    intri = dish[1]
    special = set(intri).intersection(SPECIAL_INGREDIENTS)
    return (name,special)
...
def separate_appetizers(dishes, appetizers):
    ...
    """
    appet = list(set(dishes) - set(appetizers))
    return appet
...
def singleton_ingredients(dishes, intersection):
    ...
    single = set()
    for dish in dishes:
        single = single.symmetric_difference(dish)
